   ...
   **/
   function showFormHelpdesk($ID, $ticket_template=false) {
	  ...
	  echo "<script type=\"text/javascript\"> $('#categoryTool').qtip({
         ...
         content: {text: \"Operateur : Lien DATA XDSL , VOIX<br/>Telecom : Matériel telecom (Autocom, poste, ...)<br/>Centre de contact : Lié à votre outil centre de contact, service client\"},
      ...
      if (empty($delegating)
         ...
         echo "<tr class='tab_bg_1'>";
		if (strcmp($_SESSION["glpilanguage"],"fr_FR")==0) {
...
//       "<div id='uploadfiles'><input type='file' name='filename[]' value='' size='60'></div>";
   ...
   **/
   ...
   function showForm($ID, $options=array()) {
      ...
      if ($canupdate
		 ...
		 self::dropdownCategory(array('action' => 'update' , 'id' => $ID ,'on_change' => 'this.form.submit()', 'value' => $this->fields['itilcategories_id']));
   ...
   **/
   ...
   static function showCentralList($start, $status="process", $showgrouptickets=true) {
      ...
      $query  .= " ORDER BY priority DESC , date_mod  DESC";
      $result  = $DB->query($query);
   ...
   static function showListForItem(CommonDBTM $item) {
      ...
      switch ($item->getType()) {
         ...
         default :
            ...
            $order    = '`glpi_tickets`.`priority` DESC, `glpi_tickets`.`date_mod` DESC';
